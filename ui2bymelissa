...
local Templates = {
    ...
    Window = {
        ...
        ToggleButtonAssetId = "",  -- Set to a Roblox asset ID (number/string) to show a square image toggle button
        ToggleButtonSize = 40,     -- Size in pixels of the square toggle button
    },
    Toggle = {
        Text = "Toggle",
        Default = false,
...
--// Image Toggle Button \\--
-- Creates a draggable square ImageButton that opens/closes the UI.
-- AssetId  : Roblox asset ID as a number or string (e.g. 123456789 or "rbxassetid://123456789")
-- ButtonSize: optional pixel size for the square (default 40)
function Library:AddImageToggleButton(AssetId, ButtonSize: number?)
    local Size = ButtonSize or 40
    local Table = {}

    -- Normalise the asset ID into a full rbxassetid:// URL
    local AssetUrl = tostring(AssetId):find("rbxassetid://") and tostring(AssetId)
        or ("rbxassetid://" .. tostring(AssetId):match("%d+"))
    ...
    -- Click → toggle the UI open / closed
    Button.MouseButton1Click:Connect(function()
        Library:Toggle()
    end)

    -- Allow changing the image at runtime
    function Table:SetAsset(NewAssetId)
        local Url = tostring(NewAssetId):find("rbxassetid://") and tostring(NewAssetId)
            or ("rbxassetid://" .. tostring(NewAssetId):match("%d+"))
        Button.Image = Url
    end

    -- Allow resizing at runtime
    function Table:SetSize(NewSize: number)
        Button.Size = UDim2.fromOffset(NewSize, NewSize)
    end
...
function Library:CreateWindow(WindowInfo)
    ...
    --// Square Image Toggle Button (all platforms) \\--
    -- Spawned whenever WindowInfo.ToggleButtonAssetId is provided.
    -- The button is draggable and toggles the UI open / closed on click.
    if WindowInfo.ToggleButtonAssetId and tostring(WindowInfo.ToggleButtonAssetId) ~= "" then
        local ImageToggleButton = Library:AddImageToggleButton(
            WindowInfo.ToggleButtonAssetId,
            WindowInfo.ToggleButtonSize
        )

        -- Default position: top-left corner.
        -- Shift it below the mobile Lock button when on mobile so they don't overlap.
        if Library.IsMobile then
            local YOffset = (WindowInfo.MobileButtonsSide == "Right") and 86 or 86
            if WindowInfo.MobileButtonsSide == "Right" then
                ImageToggleButton.Button.Position = UDim2.new(1, -6, 0, YOffset)
                ImageToggleButton.Button.AnchorPoint = Vector2.new(1, 0)
            else
                ImageToggleButton.Button.Position = UDim2.fromOffset(6, YOffset)
            end
        end

        Window.ImageToggleButton = ImageToggleButton
    end
